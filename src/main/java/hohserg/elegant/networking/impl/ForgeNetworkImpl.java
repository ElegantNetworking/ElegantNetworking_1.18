...
public class ForgeNetworkImpl implements Network<ForgeNetworkImpl.UniversalPacket> {
    @Override
    public void sendToPlayer(ServerToClientPacket packet, ServerPlayer player) {
    ...
    @Override
    public void sendPacketToAllAround(ServerToClientPacket packet, Level world, double x, double y, double z, double range) {
        checkSendingSide(packet);
        getChannel(packet).send(PacketDistributor.NEAR.with(() -> new PacketDistributor.TargetPoint(x, y, z, range, world.dimension())), preparePacket(packet));
    }

    @Override
    public void sendToDimension(ServerToClientPacket packet, Level world) {
        checkSendingSide(packet);
        getChannel(packet).send(PacketDistributor.DIMENSION.with(world::dimension), preparePacket(packet));
    }

    @Override
    public void sendToChunk(ServerToClientPacket packet, Level world, int chunkX, int chunkZ) {
        checkSendingSide(packet);
    ...
    @Override
    public void onReceiveServer(UniversalPacket packetRepresent, ServerPlayer player, String channel) {
